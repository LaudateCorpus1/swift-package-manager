...
fileprivate enum DirectoryNode: Codable {
    ...
    var fileAttributeType: FileAttributeType {
        ...
        case .file(_, _, let isSymlink): return isSymlink ? .typeSymbolicLink : .typeRegular
    ...
    var isSymlink: Bool {
        ...
        case .file(_, _, let isSymlink): return isSymlink
        case .root(_): return false
        }
    }
}
...
private extension FileSystem {
    func getDirectoryNodes(_ path: AbsolutePath) throws -> [DirectoryNode] {
        return try getDirectoryContents(path).compactMap {
            let current = path.appending(component: $0)
            let isSymlink = isSymlink(current)

            if isFile(current) {
                return .file(name: $0, isExecutable: isExecutableFile(current), isSymlink: isSymlink)
            } else if isDirectory(current) {
                if $0.hasPrefix(".") { return nil } // we ignore hidden files
                return .directory(name: $0, isSymlink: isSymlink, children: try getDirectoryNodes(current))
            } else {
...
public class VirtualFileSystem: FileSystem {
    ...
    private let readFileContents: ((AbsolutePath) throws -> ByteString)

    public init(path: AbsolutePath, fs: FileSystem, readFileContents: @escaping ((AbsolutePath) throws -> ByteString) = { _ in "" }) throws {
    ...
    public static func serializeDirectoryTree(_ directoryPath: AbsolutePath, into vfsPath: AbsolutePath, fs: FileSystem) throws {
        let data = try JSONEncoder.makeWithDefaults().encode(DirectoryNode.root(children: fs.getDirectoryNodes(directoryPath)))
    ...
    public func isExecutableFile(_ path: AbsolutePath) -> Bool {
        ...
        if case let .file(_, isExecutable, _) = node {
            return isExecutable
        } else {
            return false
        }
    ...
    public func readFileContents(_ path: AbsolutePath) throws -> ByteString {
        guard let node = findNode(path, followSymlink: true) else { throw Errors.noSuchFileOrDirectory(path: path) }
        if !node.isFile {
            throw Errors.notAFile(path: path)
        }
        return try readFileContents(path)
    }
