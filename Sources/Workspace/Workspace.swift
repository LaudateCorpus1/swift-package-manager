...
extension Workspace {
    ...
    public func loadDependencyManifests(
        ...
        let dependencies = try dependencyManifests.map{ identity, manifest, productFilter -> (Manifest, ManagedDependency, ProductFilter, FileSystem) in
            ...
            let container = try temp_await { self.getContainer(for: packageRef, skipUpdate: true, observabilityScope: observabilityScope, on: .sharedConcurrent, completion: $0) }
            ...
            case .fileSystem(_): fileSystem = try container.getUnversionedFileSystem()
            case .registryDownload(let version): fileSystem = try container.getFileSystem(at: version)
            }
